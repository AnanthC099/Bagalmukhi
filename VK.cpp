...
VkResult LoadTextureOnDemand(void);
			...
			break;
				...
				else
				{
					...
					fprintf(gFILE, "WndProc() WM_CHAR(F key)-> Program ended Fullscreen.\n");
                        ...
                        case 't':
                        {
                                VkResult vkResult = LoadTextureOnDemand();
                                if (vkResult != VK_SUCCESS)
                                {
                                        fprintf(gFILE, "WndProc() WM_CHAR(T key)-> LoadTextureOnDemand() failed with error code %d\\n", vkResult);
                                }
                                else
                                {
                                        fprintf(gFILE, "WndProc() WM_CHAR(T key)-> Texture load requested successfully.\\n");
                                }
                        }
                                break;
                        }
...
VkResult initialize(void)
{
	...
	*/
	...
	fprintf(gFILE, "initialize(): Texture creation deferred until key press.\n");
	...
	*/
	...
	fprintf(gFILE, "initialize(): Descriptor set will be created after texture load.\n");
	...
	*/
	...
	fprintf(gFILE, "initialize(): Command buffers will be built after texture load.\n");
...
VkResult resize(int width, int height)
{
	...
	*/
	...
	{
		fprintf(gFILE, "resize(): buildCommandBuffers() skipped because texture is not loaded yet.\n");
	}
...
void uninitialize(void)
{
	...
	fprintf(gFILE, "uninitialize(): Texture load state reset.\n");
        ...
        float quadTexcoords[] =
         {
            // front
            // triangle one
            1.0f, 0.0f, // top-right of front (Vulkan origin is top-left)
...
VkResult CreateTexture(const char* textureFileName)
{
	...
	*/
	...
	{
		fprintf(gFILE, "CreateTexture(): vkAllocateCommandBuffers() succedded for vkCommandBuffer_buffer_to_image_copy\n");
	}
...
VkResult LoadTextureOnDemand(void)
{
	...
	if(bTextureLoaded == TRUE)
	{
		fprintf(gFILE, "LoadTextureOnDemand(): Texture already loaded.\n");
		return vkResult;
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "LoadTextureOnDemand(): CreateTexture() failed with error code %d\n", vkResult);
		return vkResult;
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "LoadTextureOnDemand(): CreateDescriptorSet() failed with error code %d\n", vkResult);
		return vkResult;
	}
	...
	if (vkResult != VK_SUCCESS)
	{
		fprintf(gFILE, "LoadTextureOnDemand(): buildCommandBuffers() failed with error code %d\n", vkResult);
		return vkResult;
	}

	bTextureLoaded = TRUE;
	fprintf(gFILE, "LoadTextureOnDemand(): Texture loaded successfully on demand.\n");
